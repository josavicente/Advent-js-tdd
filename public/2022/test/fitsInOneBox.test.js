...
import { fitsInOneBox } from '../src/fitsInOneBox'
...
// Por lo tanto, es posible empaquetar todas las cajas en una sola. Ahora veamos un caso que no:
...
// // En el ejemplo anterior, la caja más pequeña entra en la caja del medio,
// pero la caja del medio no entra en la caja más grande. Por lo tanto,
...
// // En el ejemplo anterior, la primer caja cabe en la tercera, y la
// tercera en la segunda. Por lo tanto, es posible empaquetar todas las cajas en una sola.
...
describe('fitsInOneBox', () => {
  const squareBoxes = [{ l: 1, w: 1, h: 1 }, { l: 2, w: 2, h: 2 }]
  ...
  //   expect(() => fitsInOneBox()).toThrow('Parameter must be provided')
  // })
  // Cuadradas
  it('Should return true if all are square boxes and each fits in the next one', () => {
    expect(fitsInOneBox(squareBoxes)).toBe(true)
  })
})
